...
if (videoCarousel) {
  ...
  if (testimonialCarousel) {
    ...
    const moveTestimonial = direction => {
      ...
      testimonialCarousel.scrollTo({ left: nextIndex * testimonialStep(), behavior: 'smooth' });
    };
    ...
    testimonialPrev?.addEventListener('click', () => { moveTestimonial(-1); startTestimonialAutoplay(); });
    testimonialNext?.addEventListener('click', () => { moveTestimonial(1); startTestimonialAutoplay(); });
    ...
    const testimonialDialog = document.querySelector('#testimonialDialog');
    const dialogQuote = document.querySelector('#testimonialDialogQuote');
    const dialogName = document.querySelector('#testimonialDialogName');
    const dialogService = document.querySelector('#testimonialDialogService');
    const dialogClose = testimonialDialog?.querySelector('.testimonial-dialog-close');

    const openFullTestimonial = card => {
      const quote = card.querySelector('blockquote');
      const name = card.querySelector('.testimonial-client strong');
      const service = card.querySelector('.testimonial-client span');
      if (!testimonialDialog || !quote || !dialogQuote) return;

      dialogQuote.textContent = quote.textContent.trim();
      if (dialogName) dialogName.textContent = name?.textContent.trim() || 'TH3NOMADS Client';
      if (dialogService) dialogService.textContent = service?.textContent.trim() || '';
      stopTestimonialAutoplay();
    ...
    const updateReadMoreButtons = () => {
      testimonialCards.forEach(card => {
        const copy = card.querySelector('.testimonial-copy');
        const quote = card.querySelector('blockquote');
        if (!copy || !quote) return;

        let button = copy.querySelector('.testimonial-read-more');
        ...
        if (isOverflowing && !button) {
          ...
          button.setAttribute('aria-label', 'Read the full client testimonial');
          button.addEventListener('click', () => openFullTestimonial(card));
    ...
    dialogClose?.addEventListener('click', () => testimonialDialog.close());
    testimonialDialog?.addEventListener('click', event => {
      if (event.target === testimonialDialog) testimonialDialog.close();
    });
    testimonialDialog?.addEventListener('close', startTestimonialAutoplay);

    window.addEventListener('resize', () => requestAnimationFrame(updateReadMoreButtons));
