const header=document.querySelector('.site-header'); window.addEventListener('scroll',()=>header.classList.toggle('scrolled',scrollY>40)); const toggle=document.querySelector('.menu-toggle'),nav=document.querySelector('.nav-links');
...
// Fix in-page navigation when gallery images are still loading. document.querySelectorAll('a[href^="#"]').forEach(link => { link.addEventListener('click', event => { const targetId = link.getAttribute('href'); if (!targetId || targetId === '#') return; const target = document.querySelector(targetId); if (!target) return; event.preventDefault(); // Close the mobile menu if it is open. nav.classList.remove('open'); toggle.setAttribute('aria-expanded', 'false'); const scrollToTarget = () => { const headerHeight = header ? header.offsetHeight : 0; const targetPosition = target.getBoundingClientRect().top + window.scrollY - headerHeight - 12; window.scrollTo({ top: targetPosition, behavior: 'smooth' }); }; // Scroll immediately. scrollToTarget(); // Correct the position after lazy-loaded images finish adjusting the page. setTimeout(scrollToTarget, 250); setTimeout(scrollToTarget, 700); history.replaceState(null, '', targetId); }); });
